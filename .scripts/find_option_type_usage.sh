...
#
# A parameter or return value "uses Option" when the Option type appears
# anywhere in its type expression, i.e. a direct Option<T> as well as Option
# nested inside another type such as Vec<Option<T>>. Option appearing only
# inside a referenced user-defined type (struct/union/enum) is not counted,
# since that is not part of the function signature's own type expression.
#
...
JQ_PROG='
def has_opt: [.. | objects | select(has("option"))] | length > 0;
(input_filename | sub(".*/";"") | sub("\\.json$";"")) as $hash
| [ .[] | .function_v0 | select(. != null) ] as $fns
| ($fns | length) as $functions
| [ $fns[] | .inputs[] | .type_ ] as $ptypes
| [ $fns[] | .outputs[] ] as $rtypes
| ($ptypes | length) as $params
| ([ $ptypes[] | select(has_opt) ] | length) as $params_opt
| ($rtypes | length) as $returns
| ([ $rtypes[] | select(has_opt) ] | length) as $returns_opt
| ([ $fns[] | select(any(.inputs[].type_; has_opt)) ] | length) as $fwp
| ([ $fns[] | select(any(.outputs[]; has_opt)) ] | length) as $fwr
